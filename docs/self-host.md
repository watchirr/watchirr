...
services:
  watchirr:
    image: ghcr.io/watchirr/watchirr:v1.0.1
    ports:
      - "3001:3000"
    volumes:
      - ./data:/data
...
Pinned image is `ghcr.io/watchirr/watchirr:v1.0.1`. `:latest` is fine for smoke tests; check [GHCR packages](https://github.com/orgs/watchirr/packages) / releases for newer tags.
...
2. Paste `portainer-stack.yml` so that:
   ...
   - `image:` is the pinned GHCR tag (default `ghcr.io/watchirr/watchirr:v1.0.1`).
